TASK: EP-NAV-001 + EP-NAV-002 — Sidebar por role (Analytics + Administração)
...
- EDITAR: src/components/layout/mobile-menu.tsx

---

## ARQUIVO 1 — src/app/(dashboard)/layout.tsx
...
### 2a. Alterar interface SidebarProps
...
### 2b. Alterar assinatura da função
...
### 2c. Remover o bloco {showMetrics && ...} existente

Remover completamente o bloco:
```typescript
{showMetrics && (
  <div className="relative">
    <Link href="/area-usuario/metricas" ...>
      ...
    </Link>
  </div>
)}
...
### 2d. Adicionar seções novas APÓS o map de menuItems

Adicionar logo após o fechamento do `{menuItems.map(...)}`:
...
## ARQUIVO 3 — src/components/layout/mobile-menu.tsx

Inspecionar o arquivo e replicar a mesma lógica de `isAdminOrManager` e `isAdmin` que foi aplicada no sidebar.tsx.
- Adicionar as mesmas seções "Analytics" e "Administração" condicionadas por role.
- O componente provavelmente recebe props do layout; adicionar `userRole?: string` na sua interface se necessário.
- Se mobile-menu.tsx não tiver acesso ao role, passar via prop do layout.tsx.
...
DOD:
1. Admin vê Analytics (Métricas + Saúde dos Testes) e Administração (Usuários + Audit Log) na sidebar
2. tsc + lint + build sem erros · Gerente vê tudo exceto item Usuários
